  ...
  class SupportedIdentifiersByCountry
    ...
    def self.openapi_types
      {
        ...
        :'name' => :'String',
        :'flag' => :'String',
        :'individual_identifiers' => :'Array<SupportedIdentifier>',
        :'organization_identifiers' => :'Array<SupportedIdentifier>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MyDataMyConsent::SupportedIdentifiersByCountry` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MyDataMyConsent::SupportedIdentifiersByCountry`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [iso2, name, flag, individual_identifiers, organization_identifiers].hash
    end
